...
class HomeController extends Controller
{
    ...
    public function hof() {
        ...
        $top_deposits = topDeposit::all();
        $top_orders = topOrder::all();
        $news = News::orderByDesc('id')->get();
        return view('others.hall_of_fame', compact('top_sosmed', 'top_pulsa', 'top_layanan','top_deposit','top_deposits','top_orders','news'));

        // $top_deposit = topDeposit::all();
        // $top_order = topOrder::all();

        // return view('others.hall_of_fame', compact('top_order', 'top_deposit'));
    }

    public function activity() {
        $activities = Activity::where('user_id',auth()->user()->id)->orderBy('id','desc')->simplePaginate(10);
        $news = News::orderByDesc('id')->get();
        return view('others.activity', compact('activities','news'));
    }
    ...
    public function balance_usage() {
        ...
        $news = News::orderByDesc('id')->get();
        return view('others.balance_usage', compact('balance_histories','news'));
    }

    public function contact() {
        $staff = Staff::orderBy('id','asc')->get();
        $news = News::orderByDesc('id')->get();
        return view('others.contact', compact('staff','news'));
    }
    ...
    public function voucher() {
        ...
        $news = News::orderByDesc('id')->get();
        return view('others.voucher', compact('top_giveaway','news'));
    }
    ...
    public function terms() {
        $news = News::orderByDesc('id')->get();
        return view('others.terms',compact('news'));
    }

    public function requestService(){
        return view('others.request-service');
    }
