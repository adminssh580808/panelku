...
class Order_pulsa {
    ...
    public static function digiflazz($data = []) {
        $data['username'] = $data['username'] ?? "madirigZ59vW";
        $data['api_key'] = $data['api_key'] ?? 'dev-5a7abb10-164b-11ed-b6d2-95add1518c10';
        $data['sign'] = md5($data['username'] . $data['api_key'] . $data['refid']);
        ...
        $data_post['testing'] = env('DIGIFLAZZ_TESTING',true);

        $data_post = json_encode($data_post);

        $link = "https://api.digiflazz.com/v1/transaction";
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $link);
        curl_setopt($ch, CURLOPT_POST, 1);
        ...
        curl_close($ch);
        // echo $chresult;
        return  $info == 200 ? json_decode($chresult) : false;
    }
}
